...
import { ShieldAlert, Check, CheckCircle2, ArrowRight } from "lucide-react";
      ...
      openAnomalies.forEach((a: Anomaly) => {
        if (a.severity === "critical" && !toastedIds.current.has(a.anomaly_id)) {
          ...
          toast.warning(`Critical Alert: ${a.description}`, {
            ...
            action: {
              label: "Acknowledge",
              onClick: () => handleAcknowledgeAnomaly(a.anomaly_id),
            },
          });
        }
        newTasks.push({
          id: `anomaly-${a.anomaly_id}`,
          originalId: a.anomaly_id,
          type: "anomaly",
  ...
  return (
    <Card className="bg-card border-border shadow-none h-full flex flex-col">
      ...
      <CardContent className="p-lg pt-0 space-y-md flex-1 overflow-y-auto">
        {tasks.length === 0 ? (
          <div className="flex flex-row items-center justify-center py-xl text-left gap-md h-full">
      ...
      <div className="p-sm border-t border-border mt-auto">
        <Link 
          href="/tasks" 
